...
if __name__ == '__main__':
    #dbn = InstanceGroupDBN(64, 16, affine=False, momentum=1.)
    dbn = ItNInstance(16, num_groups=4, num_channels=20, T=5, affine=False, momentum=1.)
    x = torch.randn(4, 16, 4, 4)
    print(dbn)
    y = dbn(x)
    print('y size:', y.size())
    num_channels=y.size(1) // dbn.num_groups
    y = y.view(y.size(0), dbn.num_groups, num_channels, *y.size()[2:])
    y = y.view(y.size(0)*dbn.num_groups, num_channels, -1)
