...
__all__ = ['pcaWhiteningSigma', 'PCAWhiteningSigma']



class PCAWhiteningSigma_Single(torch.nn.Module):
    def __init__(self, num_features, dim=4, eps=1e-3, momentum=0.1, affine=True,
                 *args, **kwargs):
        super(PCAWhiteningSigma_Single, self).__init__()
        # assert dim == 4, 'PCAWhiteningSigma is not support 2D'
        self.eps = eps
...
class PCAWhiteningSigma(torch.nn.Module):
    def __init__(self, num_features, num_channels=16, dim=4, eps=1e-3, momentum=0.1, affine=True,
        ...
        self.PCAWhiteningSigma_Groups = torch.nn.ModuleList(
            [PCAWhiteningSigma_Single(num_features = self.num_channels, eps=eps, momentum=momentum) for _ in range(self.num_groups-1)]
        )
        num_channels_last=self.num_features - self.num_channels * (self.num_groups -1)
        self.PCAWhiteningSigma_Groups.append(PCAWhiteningSigma_Single(num_features = num_channels_last, eps=eps, momentum=momentum))

        print('PCAWhiteningSigma-------m_perGroup:' + str(self.num_channels) + '---nGroup:' + str(self.num_groups))
        self.affine = affine
        self.dim = dim
        shape = [1] * dim
        shape[1] = self.num_features
    ...
    def forward(self, X: torch.Tensor):
        ...
        for i in range(self.num_groups):
            X_hat_tmp = self.PCAWhiteningSigma_Groups[i](X_splits[i])
...
if __name__ == '__main__':
    ItN = PCAWhiteningSigma(8, num_channels=3, momentum=1, affine=False)
    print(ItN)
    ItN.train()
